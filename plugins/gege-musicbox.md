...
| 歌单连播 | 从歌单 / 列表播放时，一首结束后自动播下一首；可开列表循环 |
| 我喜欢 | 爱心收藏；需 oxmysql |
| 悬浮歌词 | 锚在音乐盒实体上方；必须视线对准才显示；字号固定 |
| 多人同步听歌 | xsound；须完成去墙 |
...
| `smoothRate` | 屏幕跟随平滑（0~1，默认 0.55；越小越稳，越大越贴盒子） |
| `facingDot` | 视线对准阈值（0~1，默认 0.62；越大越要正对音乐盒） |
...
**新增**
- 从歌单或列表播放时，当前歌曲播完后自动播放下一首
- 开启列表循环后，播到最后一首会回到第一首

### 1.3.1 · 2026-08-29

**修复**
- 去掉屏幕位置锁定，歌词重新每帧对齐音乐盒实体上方，不再固定在人物旁边
- 改用平滑跟随减轻晃动；音乐盒移动时立即重新对齐

### 1.3.0 · 2026-08-29

**优化**
- 音乐盒位置不变时锁定歌词屏幕坐标，视角左右晃不再跟着晃
- 音乐盒被移动或移出视线后再看回，会重新对齐到盒子上方

### 1.2.9 · 2026-08-29

**修复**
- 修复悬浮歌词漂在屏幕中间、不在音乐盒上方的问题
- 必须音乐盒实体在屏幕内且视线对准才显示，转头离开立即隐藏
- 每帧实时对齐音乐盒顶部，不再卡顿在旧位置

### 1.2.8 · 2026-08-29

**修复**
- 增加视线对准检测：不朝音乐盒看就不显示，转回去才再出现
- 音乐盒离开屏幕范围立即隐藏
- 对齐音箱实体顶部，减少屏幕坐标抖动
